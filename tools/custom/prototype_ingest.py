...
def _write_plane_usdpreview(out_usd: Path, basecolor_tex: Optional[Path], normal_tex: Optional[Path]) -> None:
    if HAVE_USD:
        ...
        if basecolor_tex:
            ...
            tx.CreateInput("st", Sdf.ValueTypeNames.TexCoord2f).ConnectToSource(UsdShade.Input(st.GetAttr()))
            shader.CreateInput("diffuseColor", Sdf.ValueTypeNames.Color3f).ConnectToSource(tx, "rgb")
        if normal_tex:
            ...
            ntx.CreateInput("st", Sdf.ValueTypeNames.TexCoord2f).ConnectToSource(UsdShade.Input(st.GetAttr()))
            shader.CreateInput("normal", Sdf.ValueTypeNames.Normal3f).ConnectToSource(ntx, "rgb")
